...
bool commitPart(const std::string& absPath, size_t* sizeOut) {
  ...
  if (SdMan.exists(absPath.c_str())) SdMan.remove(absPath.c_str());
  if (!SdMan.rename(part.c_str(), absPath.c_str())) return false;

  FsFile marker;
  if (!SdMan.openFileForWrite("NP-COMMIT", ok.c_str(), marker)) return false;
  const uint8_t one = '1';
  const bool markerOk = marker.write(&one, 1) == 1;
  marker.close();

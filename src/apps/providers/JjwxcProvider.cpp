  ...
  while (consumed < bodyLen) {
    ...
    while (i < n) {
      ...
      if (!entity.empty()) {
        if (b == ';' || entity.size() >= 14) {
          if (b == ';') {
            if (entity == "nbsp" || entity == "#160") emitAscii(out, ' ');
            else if (entity == "amp") emitAscii(out, '&');
            else if (entity == "lt") emitAscii(out, '<');
            else if (entity == "gt") emitAscii(out, '>');
            else emitAscii(out, ' ');
            ++i;
          } else {
            emitAscii(out, '&');
            for (char c : entity) emitAscii(out, c);
          }
          entity.clear();
          continue;
        }
      ...
      if (b == '&') {
        ...
        entity.push_back('\0');
        entity.clear();
        // Empty string also means inactive, so use a sentinel then erase it by
        // seeding the first entity character on the next byte.
        entity = "@";
        ++i;
        continue;
      }

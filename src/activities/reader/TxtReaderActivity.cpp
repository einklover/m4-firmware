...
bool TxtReaderActivity::switchToProviderChapter(const std::string& cacheRelPath, int index0,
  ...
  // Prefetch N+1 for the chapter we just entered.
  (void)M4ContentProviderSession::requestPrefetch(pluginSession_.providerId, pluginSession_.bookId, index0 + 1);
...
bool TxtReaderActivity::tryProviderNextChapterAdvance() {
  ...
  if (d.shouldRequestPrefetch) {
    (void)M4ContentProviderSession::requestPrefetch(pluginSession_.providerId, pluginSession_.bookId, next);
  }
    ...
    if (updateRequired) {
      ...
      if (lockState(0)) {
        ...
        if (doIndex) {
          ...
          const int added = continuePageIndex(3, 48 * 1024);
...
bool TxtReaderActivity::loadPageAtOffset(size_t offset, size_t endOffset, std::vector<std::string>& outLines,
  ...
  const int asciiSpaceWidth = renderer.getTextWidth(cachedFontId, " ");
  ...
  // 缓存 ASCII 可见字符宽度 (0x21-0x7E)
  int asciiWidthCache[128];
  memset(asciiWidthCache, 0, sizeof(asciiWidthCache));
  {
    char tmp[2] = {0, 0};
    for (int c = 0x20; c <= 0x7E; c++) {
      tmp[0] = (char)c;
      asciiWidthCache[c] = renderer.getTextWidth(cachedFontId, tmp);
    }
  }

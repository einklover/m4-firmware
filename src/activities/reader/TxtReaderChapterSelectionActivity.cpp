...
void TxtReaderChapterSelectionActivity::onExit() {
  ...
  // Stop display task work first. Paint path uses cache-only (no multi-second
  // scan). Prefetch may still be scanning — wait briefly for displayBusy_.
  finished_ = true;
  updateRequired = false;
  Serial.printf("[ChapterSel] onExit begin busy=%d\n", displayBusy_ ? 1 : 0);

  for (int i = 0; i < 100 && displayBusy_; ++i) {  // up to ~2s
    vTaskDelay(20 / portTICK_PERIOD_MS);
  }
  if (displayBusy_) {
    Serial.printf("[ChapterSel] onExit still busy — force-delete display task\n");
  }

  if (renderingMutex) {
    xSemaphoreTake(renderingMutex, pdMS_TO_TICKS(200));
  }
...
void TxtReaderChapterSelectionActivity::displayTaskLoop() {
  while (true) {
    // After select/back, park until onExit deletes this task.
    if (finished_) {
      displayBusy_ = false;
      vTaskDelay(20 / portTICK_PERIOD_MS);
      continue;

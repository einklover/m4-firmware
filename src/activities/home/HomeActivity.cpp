...
void HomeActivity::onEnter() {
  ...
  // First paint of each home entry uses HALF (clears reader AA residual without
  // looking like a full flash storm). Later home redraws stay FAST.
...
void HomeActivity::render() {
  ...
  // After reader with AA, FAST alone can flash residual gray plane as a "全刷".
  // Prefer HALF when first painting home after a cold entry (firstRenderDone false
  // or cover not yet settled).
  if (!firstRenderDone) {

...
void CrossPointWebServer::handleRoot() const {
  // 使用 send_P 发送 PROGMEM 中的内容，避免 String 转换失败
...
void CrossPointWebServer::handleNotFound() const {
  ...
  // 忽略 WebSocket 升级请求（ws:// 或 wss:// 前缀）
  if (uri.isEmpty() || uri == "/ws") {
    server->send(404, "text/plain", "Not Found");
    return;
  }
  
  // 其他未知请求才打印日志
  Serial.printf("[%lu] [WEB] 404 Not Found: %s\n", millis(), uri.c_str());
  ...
  for (const auto& s : settings) {
    ...
    switch (s.type) {
      ...
      case SettingType::VALUE: {
        ...
        doc["min"] = static_cast<int>(s.valueRange.min);
        doc["max"] = static_cast<int>(s.valueRange.max);
        doc["step"] = static_cast<int>(s.valueRange.step);
        break;
      }
...
void CrossPointWebServer::handlePostSettings() {
  ...
  for (auto& s : settings) {
    ...
    switch (s.type) {
      ...
      case SettingType::VALUE: {
        const int val = doc[s.key].as<int>();
        if (val >= static_cast<int>(s.valueRange.min) && val <= static_cast<int>(s.valueRange.max)) {
          if (s.signedValuePtr) {

...
void FengyanTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  int rowHeight =
      (rowSubtitle != nullptr) ? FengyanMetrics::values.listWithSubtitleRowHeight : FengyanMetrics::values.listRowHeight;
  ...
  const int iconYOffset = M4UiText::listIconTop(renderer, UI_10_FONT_ID, rowHeight,
                                                rowSubtitle != nullptr, listIconSize);
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    if (rowSubtitle != nullptr) {
      ...
      M4UiText::draw(renderer, UI_10_FONT_ID, textX, itemY + 30, subtitle.c_str(), true);
    }

...
constexpr uint32_t kTagGvar = 0x67766172;  // 'gvar'  -> variable font, rejected
constexpr uint32_t kTagFvar = 0x66766172;  // 'fvar'  -> variable font, rejected
...
bool TtfFont::init(TtfStream& s) {
  ...
  // Variable-font rejection: presence of gvar/fvar means glyf outlines are
  // interpolated at runtime; reading the default outline would render wrong.
  bool variable = false;
  for (uint16_t i = 0; i < numTables; i++) {
    ...
    if (key == kTagGvar || key == kTagFvar) variable = true;
  }
  if (variable) {
    lastError_ = "variable font not supported";
    return false;
  }
